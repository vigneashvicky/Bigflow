...
def add_schdle(request):
    if request.method == 'POST':
        ...
        if jsondata.get('parms').get('TYPE') == 'SCHEDULE':
            ...
            obj_add_schedule.entity_gid = request.session['Entity_gid']
    ...
    if request.method == 'POST':
        ...
        if obj_preschedule_get.action == 'reportview':
            ...
            obj_preschedule_get.date = datetime.datetime.strptime(jsondata.get('parms').get('f_date'),
                                                                  "%d/%m/%Y").strftime(
                "%Y-%m-%d")
            obj_preschedule_get.jsonData = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
            ...
            obj_preschedule_get.date = datetime.datetime.strptime(jsondata.get('parms').get('f_date'),
                                                                  "%d/%m/%Y").strftime(
                "%Y-%m-%d")
            obj_preschedule_get.jsonData = json.dumps({"entity_gid": [request.session['Entity_gid']],
                                                       "client_gid": [jsondata.get('parms').get('Client_gid')]})
    ...
    if request.method == 'GET':
        ...
        if (request.GET['localaction'] == 'noexcel'):
            ...
            obj_empvsschedule.jsondata = json.dumps(
                {"entity_gid": [request.session['Entity_gid']], "client_gid": [request.GET['Cname']]})
...
def sales_order_set(request):
    if request.method == 'POST':
        ...
        if jsondata.get('parms').get('ACTION') == 'Insert':
            if jsondata.get('parms').get('emp_gid') > 0:
                obj_sales.employee_gid = jsondata.get('parms').get('emp_gid')
...
def get_addScheduleDtl(request):
    ...
    if request.method == 'GET':
        ...
        obj_mapped_customer1.jsonData = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
...
def sales_order_get(request):
    if request.method == 'POST':
        ...
        obj_salesget.jsonData = json.dumps(
            {"Entity_Gid": [request.session['Entity_gid']], "Client_Gid": [jsondata.get('Client_gid')]})
...
def collection_set(request):
    if request.method == 'POST':
        ...
        obj_add_collection.entity_gid = request.session['Entity_gid']
...
def getOutstanding(request):
    if request.method == "POST":
        ...
        headers = {"content-type": "application/json",
                   "Authorization": "" + token + ""}
        params = {'ACTION': "Common", 'Entity_gid': "" + str(request.session['Entity_gid']) + "",
                  'Limit': "" + str(limit) + ""}
...
def leadrequest_set(request):
    if request.method == 'POST':
        ...
        obj_leadsrequest.entity_gid = request.session['Entity_gid']
            ...
            if (schedule_type == 'COLLECTION'):
                if (pd.notnull(Schedule_ref_gid)):
                    ...
                    obj_Schdul_det.jsondata = json.dumps(
                        {"entity_gid": [request.session['Entity_gid']], "client_gid": []})
...
def getCustomerFilterlist(request):
    if request.method == 'GET':
        ...
        obj_hirEmpList.jsonData = json.dumps(
            {"entity_gid": [request.session['Entity_gid']], "client_gid": [(request.GET['Client_gid'])]})
        ...
        obj_clustertree.jsondata = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
        ...
        obj_hirEmpList.jsonData = json.dumps(
            {'FILTER': [{'Employee_gid': employee, 'Cluster_gid': clusterdtl, 'Route_header_gid': routedtl}]})
        obj_hirEmpList.jsondata = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
        ...
        obj_cltn.Classification = json.dumps({"Entity_Gid": [int(request.session['Entity_gid'])]})
...
def sale_approval(request):
    if request.method == 'POST':
        ...
        obj_lead_approve.entity_gid = request.session['Entity_gid']
...
def sale_order_get(request):
    if request.method == 'POST':
        ...
        params['Entity_Gid'] = request.session['Entity_gid']
...
def snap_shot_get(request):
    ...
    if request.method == 'GET':
        ...
        obj_laglong.entity_gid = json.dumps(request.session['Entity_gid'])
        resp = requests.get(
...
def get_collectionapi(request):
    ...
    params = {'Type': "" + type + "", 'Sub_Type': "" + subtype + ""}
    headers = {"content-type": "application/json", "Authorization": "" + token + ""}
    datas = json.dumps(jsondata.get('darta'))
...
def setschedule_review(request):
    if request.method == 'POST':
        ...
        obj_schReview.entity_gid = request.session['Entity_gid']
...
def fetreview_getexcel(request):
    try:
        if request.method == 'GET':
            ...
            obj_review.jsonData = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
...
def fetemployee_getexcel(request):
    try:
        if request.method == 'GET':
            ...
            XLSX_MIME = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            response = HttpResponse(content_type=XLSX_MIME)
            response['Content-Disposition'] = 'attachment; filename="Exective tracking Excel.xlsx"'
            writer = pd.ExcelWriter(response, engine='xlsxwriter')
...
def colset(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Group')) == 'SET_INITIAL':
            ...
            obj.entgid = json.dumps(jsondata.get('Entity_Gid'))
            obj.colgid = json.dumps(jsondata.get('Collection_Gid'))
            dataa = json.dumps(jsondata.get("Params"))
            resp = requests.post(
                ip + "/FET_Collection_API?Action=" + obj.actn + "&Type=" + obj.typ + "&Group=" + obj.grp + "&Customer_Gid=" + obj.cusgid + "&Employee_Gid=" + obj.empgid + "&Collection_Mode=" + obj.colmode + "&Collection_Amount=" + obj.colamt + "&Collection_Date=" + obj.coldat + "&Collection_Description=" + obj.coldes + "&Entity_Gid=" + obj.entgid + "&Collection_Gid=" + obj.colgid + "",
                data=dataa,
                headers={"content-type": "application/json",
                         "Authorization": "" + token + ""
                         }, verify=False)
            ...
            datas = json.dumps(jsondata.get('data'))
            resp = requests.post(
                "" + ip + "/FET_Collection_API?Action=" + obj.actn + "&Type=" + obj.typ + "&commit=" + obj.cmt + "&Create_by=" + obj.cretby + "&Group=" + obj.grp + "",
                data=datas,
                headers={"content-type": "application/json",
                         "Authorization": "" + token + ""
                         }, verify=False)
            ...
            obj.typ = jsondata.get('Type')
            obj.empgid = json.dumps(jsondata.get('Employee_Gid'))
            obj.entitygid = json.dumps(jsondata.get('Entity_Gid'))
            obj.grp = jsondata.get('Group')
            datas = json.dumps(jsondata.get('data'))
            resp = requests.post(
                "" + ip + "/FET_Collection_API?Action=" + obj.actn + "&Type=" + obj.typ + "&Group=" + obj.grp + "&Employee_Gid=" + obj.empgid + "&Entity_Gid=" + obj.entitygid + "",
                data=datas,
                headers={"content-type": "application/json",
                         "Authorization": "" + token + ""
                         }, verify=False)
            ...
            datas = {}
            resp = requests.post(
                "" + ip + "/FET_Collection_API?Action=" + obj.actn + "&Type=" + obj.typ + "&Group=" + obj.grp + "&Collection_Gid=" + obj.headergid +"&Employee_Gid=" + obj.empgid + "&Entity_Gid=" + obj.entitygid + "",
                data=datas,
                headers={"content-type": "application/json",
                         "Authorization": "" + token + ""
                         }, verify=False)
            ...
            datas = json.dumps(jsondata.get('data'))
            resp = requests.post(
                "" + ip + "/FET_Collection_API?Action=" + obj.actn + "&Type=" + obj.typ + "&Create_by=" + obj.crtby + "&Group=" + obj.grp + "&In_Out=" + obj.inout + "&Courier_Gid=" + obj.couid + "&Dispatch_Date=" + obj.ddate + "&Send_By=" + obj.sndby + "&AWB_No=" + obj.awbn + "&Dispatch_Mode=" + obj.dmode + "&Packets=" + obj.pack + "&Weight=" + obj.wght + "&Dispatch_To=" + obj.dto + "&Address=" + obj.adrs + "&City=" + obj.cty + "&State=" + obj.stat + "&Pincode=" + obj.pcode + "&Remark=" + obj.rmrk + "&Returned=" + obj.retrnd + "&Status=" + obj.stts + "&Entity_Gid=" + obj.entitygid + "&Employee_Gid=" + obj.empgid + "",
                data=datas,
                headers={"content-type": "application/json",
                         "Authorization": "" + token + ""
                         }, verify=False)
...
def custgroupget(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Group')) == 'CUST_GROUP_GET':
            obj = mSales.Sales_Model()
            obj.grp = jsondata.get('Group')
            obj.srvrip = common.serverip()
            obj.entygid = json.dumps(jsondata.get('Entity_Gid'))
...
def query_summary_salesget(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'QUERYSUMMARY':
            ...
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/apiurl_Query_SummarySales", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def query_summary_salesproductget(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'QUERYSUMMARY':
            ...
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/apiurl_Query_SummarySalesProduct", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def query_summary_collectionget(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'QUERYSUMMARY':
            ...
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/apiurl_Query_SummaryCollection", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def query_summary_outstandingget(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'QUERYSUMMARY':
            ...
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/apiurl_Query_SummaryOutstanding", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def query_summary_getcollectionstatus(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'QUERYSUMMARY':
            ...
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/apiurl_query_summary_getcollectionstatus", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def gettadetails(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Params').get('Group')) == 'CLAIM_APPROVE_PROCESS':
            ...
            params = {'Group': "" + grp + "", 'Type': "" + typ + "", 'Sub_Type': "" + sbtyp + "", 'Action': "" + action + "", 'Employee_Gid': "" + empgid + ""}
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata)
            resp = requests.post("" + ip + "/Claim_Initial", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
...
def getcustemp(request):
    if request.method == 'GET':
        ...
        obj.emp_gid = json.dumps(request.session['Emp_gid'])
        obj.entity = json.dumps(request.session['Entity_gid'])
        resp = requests.get(
...
def custmer_get(request):
    if request.method == 'POST':
        ...
        if (jsondata.get('Group')) == 'CUST_All_GET':
            ...
            params = {'Group':""+jsondata.get('Group')+"",'Type': "" + obj.typ + "", 'Sub_Type': "" + obj.sbtyp + "",'Limit':""+ obj.limit+""}
            headers = {"content-type": "application/json", "Authorization": "" + token + ""}
            datas = json.dumps(jsondata.get('data'))
            resp = requests.post("" + ip + "/Customer_Get_API", params=params, data=datas, headers=headers, verify=False)
            response = resp.content.decode("utf-8")
            return HttpResponse(response)
...
def Productsmry(request):
  if request.method == 'POST':
      ...
      if (jsondata.get('Group')) == "Product_Get":
          ...
          params = {'Group': "" + form.grp + "", 'Type': "" + form.type + "", 'SubType': "" + form.subtype + ""}
          headers = {"content-type": "application/json", "Authorization": "" + token + ""}
          datas = json.dumps(jsondata.get('data'))
          resp = requests.post("" + ip + "/Product_Master", params=params, data=datas, headers=headers, verify=False)
          response = resp.content.decode("utf-8")
          return HttpResponse(response)
...
def producttypeset(request):
 if request.method == 'POST':
     jsondata = json.loads(request.body.decode('utf-8'))
     if (jsondata.get('Group')) == 'Product_Type' or 'Product_Carton_Map' or 'insert_data' or 'Product_Category':
         ...
         headers = {"content-type": "application/json", "Authorization": "" + token + ""}
         datas = json.dumps(jsondata.get('data'))
         resp = requests.post("" + ip + "/ProductCat_type_Set", params=params, data=datas, headers=headers, verify=False)
         response = resp.content.decode("utf-8")
         return HttpResponse(response)
...
def scanning_barcode(request):
    if request.method == 'POST':
        ...
        param = {'Group': "" + 'CLAIM_INITIAL_SET' + "", 'Action': "" + 'Insert' + "",
                 'Type': "" + 'CLAIM_INITIAL' + "", 'Sub_Type': "" + 'TEMP' + "", 'Employee_Gid':+ employee_gid
                 }
        resultdata = {
            "Params": {
                "File": [{
        ...
        subpath = (settings.MEDIA_ROOT) + '/INWARD/' + str(current_year_full)+ '/' + str(current_month) + '/' + str(current_day) + '/' + str(employee_gid)
        f = subpath + '/Tempinward/'
        if int(filecount)==int(pdfcount):
            ...
            try:
                for i in l:
                    ...
                    # base_filename = os.path.splitext(os.path.basename(i))[0] + '.jpg'
                    print(barcodes)
                    inbarnum = 0
                    inbarchar = 0
            ...
            for i in range(len(o)):
                pdf = FPDF()
                count2 = count2 + 1
                for image in o[i]['image']:
                    print(image)
                    pdf.add_page()
                    pdf.image(image, 10, 10, 190)
                    pdffilename = str(o[i]['barcodename'])
                    # removefile(image)
