CREATE DEFINER=`developer`@`%` PROCEDURE `sp_FA_Value_Set`(IN `ls_Action` varchar(32),
					...
					set Query_Insert = concat('
							...
							DEALLOCATE PREPARE stmt;
                              ...
                              if countRow > 0 then
                                  ...
                                  where assetdetails_trangid = @Asset_Gid;
                                  ...
                                  set countRow = (select ROW_COUNT());
									...
									set Message = 'SUCCESS';
                                    ## TO DO
						/*call sp_Trans_Set('Insert','FA_WRITEOFF',@Asset_Value_Maxgid,'NEW',
										  'G','MAKER',@Asset_Value_Reason,@Entity_Gid,ls_Createby,@message);
...
elseif ls_Type = 'VALUE_CHECKER' and  ls_Sub_Type = 'CHECKER' then
         if ls_Action = 'UPDATE' then
               ...
               set Query_Update = concat('Update fa_trn_tassetvalue
                  ...
                  select assetvalue_assetdetailsid from fa_trn_tassetvalue
				  where assetvalue_assetdetailsid= @Asset_Value_Gid into @AssetDetails_Id ;

			set Query_Update = '';
			set Query_Update = concat('delete from fa_tmp_tassetdetails
											where assetdetails_id=',@AssetDetails_Id,' ');
									...
									if countRow <= 0 then
										set Message = 'Error On Value fa_tmp Deletion .';
										rollback;
										leave sp_FA_TFR_Asset_Set;
									 elseif    countRow > 0 then
										set Message = 'SUCCESS';
