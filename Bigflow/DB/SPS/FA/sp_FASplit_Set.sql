CREATE DEFINER=`developer`@`%` PROCEDURE `sp_FASplit_Set`(IN `ls_Action` varchar(32),
...
if ls_Type = 'ASSET_SPLIT' and ls_Sub_Type = 'HEADER' then
 ...
 elseif ls_Type = 'ASSET_SPLIT' and ls_Sub_Type = 'DETAIL' then
					...
					set Query_Insert  = concat('insert into fa_trn_tassetsplit (assetsplit_assetsplitheader_gid,assetsplit_newassetdetailsid,assetsplit_value,
													entity_gid,create_by)
													values (''',@Asset_Header_Gid,''',''',@Asset_New_Id,''',''',@Split_Value,''',''',@Entity_Gid,''',''',ls_Createby,'''
                                                    )  ');
                    ...
                    set Query_Insert = '';
						 set Query_Insert = concat('
						  INSERT INTO fa_tmp_tassetdetails
							...
							assetdetails_imagepath,assetdetails_vendorname,assetdetails_trangid,
