CREATE DEFINER=`developer`@`%` PROCEDURE `sp_FAAsset_Set`(IN `ls_Action` varchar(32),IN `ls_Type` varchar(32),
...
if ls_Type = 'ASSET_INITIAL' and ls_Sub_Type = 'TRAN' then
         ...
         set Query_Insert = concat('
          INSERT INTO fa_trn_tassetdetails
            ...
            assetdetails_branchgid,assetdetails_assetlocationgid,assetdetails_deponhold,assetdetails_deprate,assetdetails_parentgid,assetdetails_assetserialno,assetdetails_invoicegid,
			...
			assetdetails_imagepath,assetdetails_vendorname,assetdetails_isactive,assetdetails_isremoved,entity_gid,create_by)
				...
				assetdetails_branchgid,assetdetails_assetlocationgid,assetdetails_deponhold,assetdetails_deprate,assetdetails_parentgid,assetdetails_assetserialno,
...
elseif ls_Type = 'ASSET_CHECKER' and ls_Sub_Type = 'APPROVE' then
		 ...
		 #select @AssetDetails_Tran_Gid,@AssetDetails_Status,@AssetDetails_RequestStatus,@Asset_Details_Gids,@Request_For;
		 set Query_Update = '';
...
elseif ls_Type = 'ASSET_PROCESS_CHECKER' and ls_Sub_Type = 'APPROVE' then
	   ...
	   SET SQL_SAFE_UPDATES = 0;
       Delete from fa_tmp_tassetdetails where assetdetails_trangid in (@Asset_Details_Gids);
...
elseif ls_Type = 'ASSET_PROCESS_CHECKER' and ls_Sub_Type = 'REJECT' then
 ...
 elseif ls_Type = 'ASSET_CHECKER' and ls_Sub_Type = 'REQUEST_FOR' and ls_Action='UPDATE' then
     ...
     #REQUEST_FOR  UPDATE
                ...
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_Value_Gid'))) into @Asset_Value_Gid; ### Only for Asset_Transfer_Gid
                ...
                if @Asset_Value_Gid is not null and @Asset_Value_Gid <> 0 then
					set Query_Update1 = Concat(Query_Update1,' ,assetdetails_assetvaluegid = ''',@Asset_Value_Gid,'''  ');
                End if;

                if @Asset_Merge_Gid is not null and @Asset_Merge_Gid <> 0 then
					set Query_Update1 = Concat(Query_Update1,' ,assetdetails_assetmergegid = ''',@Asset_Merge_Gid,'''  ');
                End if;
 ...
 elseif ls_Type = 'ASSET_CHECKER' and ls_Sub_Type = 'PARENT' and ls_Action='UPDATE' then
		...
		select JSON_LENGTH(lj_Details,'$') into @li_Detailjson_count;
				...
				SET @Asset_Tran_Gids='';
                    select group_concat(assetdetails_trangid) from fa_tmp_tassetdetails
						   where assetdetails_parentgid in (@Asset_Parent_Gids) INTO @Asset_Detail_Tran_Gids ;


					select assetdetails_trangid from fa_tmp_tassetdetails
						   where assetdetails_gid in (@Asset_Parent_Gids)  INTO @Asset_Tran_Gid;

                      # select @Asset_Tran_Gid;
                      # select @Asset_Detail_Tran_Gids;
			...
			set Query_Update = concat('update fa_trn_tassetdetails
										set
											update_date=now(),
											update_by=',ls_Createby,',
											assetdetails_parentgid=',@Asset_Tran_Gid,'
