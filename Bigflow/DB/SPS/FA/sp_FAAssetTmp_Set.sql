CREATE DEFINER=`developer`@`%` PROCEDURE `sp_FAAssetTmp_Set`(IN `ls_Action` varchar(32),IN `ls_Type` varchar(32),IN `ls_Sub_Type` varchar(32),
...
Declare Asset_Qty int;
 ...
 ### Validation Pending
    ...
    While i <= Asset_Qty - 1 Do
	   ...
	   #select DATE_ADD(b.assetdetails_capdate, INTERVAL a.assetcat_lifetime YEAR)
			...
			#where a.assetdetails_isactive='Y' and a.assetdetails_isactive='Y' and a.entity_gid in ('',@Entity_Gid,'') and
				 # b.assetcat_isactive='Y' and b.assetcat_isremoved='Y' and b.entity_gid in ('',@Entity_Gid,'')  ;


					Select ifnull(max(cast(assetdetails_id as decimal)),0)+1 into @Asset_Id from fa_tmp_tassetdetails;
					set Query_Insert = '';
					set Query_Insert = concat('INSERT INTO fa_tmp_tassetdetails (assetdetails_id, assetdetails_productgid,assetdetails_qty, assetdetails_barcode, assetdetails_date, assetdetails_assetgroupid,
								assetdetails_assetcatgid, assetdetails_cat, assetdetails_subcat, assetdetails_value, assetdetails_cost, assetdetails_description, assetdetails_capdate,
                                assetdetails_source, assetdetails_status, assetdetails_requestfor,assetdetails_requeststatus,assetdetails_assetowner,assetdetails_branchgid,assetdetails_ponum,
                                assetdetails_crnum,assetdetails_vendorname,',Query_Column,'entity_gid,create_by)
								VALUES (''',@Asset_Id,''', ''',@Asset_Product_Gid,''',''',@Asset_QtyTmp,''', ''',@Asset_Barcode,''', ''',@Asset_Date,''', ''',@Asset_Group_Id,''', ''',@Asset_Asset_SubCat_Id,''', ''',@Asset_Cat_Id,''',
                                ''',@Asset_SubCat_Id,''',''',@Asset_Value,''', ''',@Asset_Cost,''', ''',@Asset_Description,''', ''',@Asset_CapDate,''', ''',@Asset_Source,''', ''',@Asset_Status,''', ''',@Asset_RequestFor,''',
								''',@Asset_RequestStatus,''',''',@Asset_AssetOwner,''',''',@Asset_Branch_Gid,''',''',@Asset_PONo,''',''',@Asset_CRNo,''',''',@Asset_VendorName,''',',Query_Value,'''',@Entity_Gid,''',''',ls_Createby,''')');
				...
				call sp_Trans_Set('Insert',@Trn_Ref_Name,@Trn_Reftable_Gid,
										 @Asset_Status,@Trn_To_Type, @Trn_Role_Name,
        ...
        set @lj_Details_Data = '';	#Asset_Detail_Gids#Trn_Reftable_Gid
               ...
               #select @Assetdetails_Trn_Gid;
				...
				set assetdetails_trangid= @Assetdetails_Trn_Gid
 ...
 elseif ls_Type = 'GROUP' and ls_Sub_Type = 'INITIAL' then
               select JSON_LENGTH(lj_Details,'$') into @li_Group_count;
                ...
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_SubCat_Gid'))) into @Asset_SubCat_Gid;
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_CPDate'))) into @Asset_CPDate;
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_Value'))) into @Asset_Value;
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_Branch_Gid'))) into @Asset_Branch_Gid;
                select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Asset_Group_MaxNo'))) into @Asset_Group_MaxNo;
           ...
           set Query_Insert = concat('INSERT INTO fa_trn_tassetgroup (assetgroup_no,assetdetails_cat,assetdetails_subcat,assetgroup_capdate,
                                                     ...
                                                     values (''',@Asset_Group_MaxNo,''',''',@Asset_Cat_Gid,''',''',@Asset_SubCat_Gid,''',''',@Asset_CPDate,''',
                                                     ''',@Asset_Value,''',''',@Asset_Branch_Gid,''',''',@Entity_Gid,''',''',ls_Createby,'''
                                                     )
													 ');
 ...
 elseif ls_Type = 'ASSET_PARENT' and ls_Sub_Type = 'MAKER' then
						...
						select JSON_UNQUOTE(JSON_EXTRACT(lj_Details, CONCAT('$.Request_Status'))) into @ls_Request_Status;
						 ...
						 set Query_Insert = concat('
						  INSERT INTO fa_tmp_tassetdetails
                            ...
                            assetdetails_reducedvalue,assetdetails_branchgid,assetdetails_deponhold,assetdetails_deprate,assetdetails_parentgid,
                            ...
                            assetdetails_ponum,
                            assetdetails_crnum,assetdetails_imagepath,assetdetails_vendorname,assetdetails_trangid,assetdetails_isactive,
                            ...
                            entity_gid,create_by,assetdetails_assetlocationgid)
								(select assetdetails_id,assetdetails_qty,assetdetails_barcode,assetdetails_date,assetdetails_assetgroupid,
									...
									assetdetails_branchgid,assetdetails_deponhold,assetdetails_deprate,''',@AssetDetails_Parent_Gid_Tmp,''',
                        ...
                        set @Parent_Gid='';
							...
							where assetdetails_trangid in (@AssetDetails_Parent_Gid_Tmp) into @Parent_Gid;
                       ...
                       if @Parent_Gid='' or @Parent_Gid is null then
						...
						set Query_Update = concat('Update fa_tmp_tassetdetails
													  ...
													  where assetdetails_trangid in (',@AssetDetails_Gid,')
												  ');
                        ...
                        #select @ls_Status,@ls_Request_For,@ls_Request_Status,@Asset_Detail_Gids;
						 ...
						 set Query_Insert = concat('
						  INSERT INTO fa_tmp_tassetdetails
							...
							assetdetails_imagepath,assetdetails_vendorname,assetdetails_trangid,assetdetails_isactive,assetdetails_isremoved,
                            entity_gid,create_by,assetdetails_assetlocationgid)
                              ...
                              if @ls_Request_For = 'CAPDATE'  or @ls_Request_For = 'ASSETCAT' then
                                   ...
                                   # select @New_cat_gid,@Asset_CPDate_NewD,@Dep_ValueD;
                                    ...
                                    if @Dep_ValueD <> 0 then
										...
										Select a.assetdetails_cost,a.assetdetails_value,b.assetcat_deptype,b.assetcat_deprate,b.assetcat_lifetime
                                  ...
                                  # select @DepValueTotalD,@Dep_ValueD;


										   set sql_safe_updates = 0 ;
										update fa_tmp_tassetdetails set assetdetails_cost = (assetdetails_cost  - @DepValueTotalD) where assetdetails_trangid = @Asset_Detail_Gids ;
				...
				End if;
						 ...
						 set Query_Update = concat('update fa_tmp_tassetdetails
														   set update_by=''',ls_Createby,''',
																update_date=''',Now(),'''
																',Query_Column,'
															where assetdetails_trangid in (',@Asset_Tran_Gids,')
