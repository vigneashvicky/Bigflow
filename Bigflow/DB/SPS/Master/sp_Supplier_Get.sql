CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_Supplier_Get`(IN `ls_Type` varchar(32),IN `ls_SubType` varchar(32),
  ...
  if ls_Type='SUMMARY' and ls_SubType='SUPPLIER' then
				...
				if @li_json_count is not null or @li_json_count <> '' then
				 ...
				 if @Supplier_code is not null or @Supplier_code <> 0 or @Supplier_code<>'' then
					...
					set Query_Select=concat('select supplier_gid,supplier_isactive,supplier_name,supplier_capacity,supplier_code,address_gid,address_ref_code,address_1,address_2,address_3,address_pincode,
					...
					contact_landline,contact_landline2,contact_mobileno,contact_mobileno2,contact_email,date_format(contact_Dob,''%Y-%m-%d '') as contact_Dob ,date_format(contact_Wd,''%Y-%m-%d '') as contact_Wd from gal_mst_tsupplier as a
   ...
   elseif ls_Type='DDL' and ls_SubType='SUPPLIER_GROUP' then
            Select customergroup_gid as suppliergroup_gid,customergroup_name as suppliergroup_name from gal_mst_tcustomergroup;
            set Message = 'FOUND';
  end if;
END