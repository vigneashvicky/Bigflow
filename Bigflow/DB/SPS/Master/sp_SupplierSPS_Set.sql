CREATE DEFINER=`developer`@`%` PROCEDURE `sp_SupplierSPS_Set`(IN `Action` varchar(32),IN `lj_supplier` json,
...
if Action='SUPP_INSERT' OR Action = 'SUPPLIER_GROUP_INSERT' then
				...
				select JSON_LENGTH(lj_address,'$')into @li_address_count;
				 ...
				 if  @li_contact_count is not null or @li_contact_count <>'' then
						 ...
						 #select @contacttype_gid,@contact_name,@desig_gid,@landline_no1,@landline_no2, @mobile_no1,@mobile_no2,
						 #@email,@contact_dob,@contact_wd,@Entity_gid,@Create_by;

						   #select @supplier_contact_gid;
						   if @supplier_contact_gid is not null and @supplier_contact_gid<>'0' then
								set @contact_max_gid=@supplier_contact_gid;
								#select @contact_max_gid;
						   else
					 ...
					 if @contact_wd is null then
					   ...
					   if Action = 'SUPP_UPDATE' then
						   ...
						   if @Supplier_capacity is null  then
							...
							set @query_insert = '';
							set @query_insert =concat('update gal_mst_tsupplier set supplier_name=''',@Supplier_name,''',supplier_capacity=''',@Supplier_capacity,''',
                                                       supplier_groupgid = ''',@Supplier_Group_Gid,''',supplier_branchname = ''',@Supplier_Branch_Name,''',supplier_gstno = ''',@Supplier_GST_No,''',
