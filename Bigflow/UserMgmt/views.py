...
def get_roleList(request):
    if request.method == 'GET':
        ...
        obj_roleList.entity_gid = json.dumps({"Entity_Gid": request.session['Entity_gid']})
...
def get_userList(request):
    if request.method == 'GET':
        ...
        obj_userList.json_data = json.dumps({"Entity_Gid": request.session['Entity_gid']})
...
def get_menuList(request):
    if request.method == 'GET':
        ...
        df = pd.DataFrame({'menu_gid':['N','Y'],
                    'menu_name':['Web','Andriod']}, index = [0,1])
        mainlist = []
...
def getEmployeeDtl(request):
    if request.method == 'GET':
        ...
        obj_empList.jsonData = json.dumps({"entity_gid": [request.session['Entity_gid']], "client_gid": []})
